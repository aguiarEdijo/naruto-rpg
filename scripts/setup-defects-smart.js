...
async function migrateDefects() {
    ...
    try {
        ...
        const testData = {
            ...
            tipo: 'Teste',
            descricao: 'Teste temporário'
        };

        console.log('🧪 Testando inserção para verificar colunas disponíveis...');
        ...
        if (testError) {
            ...
            // Se erro for de coluna não encontrada, vamos tentar apenas com nome
            if (testError.message.includes('Could not find the') && testError.message.includes('column')) {
                console.log('🔧 Tentando inserir apenas com coluna nome...');
                ...
                if (simpleError) {
                    console.error('❌ Erro mesmo com dados simples:', simpleError);
                    throw simpleError;
                }

                console.log('✅ Inserção simples funcionou! A tabela só tem coluna nome.');
...
CREATE INDEX IF NOT EXISTS idx_defects_nome ON defects(nome);
                ...
                throw new Error('Colunas tipo e descricao não existem na tabela');
            }

            throw testError;
        }

        console.log('✅ Teste de inserção funcionou! Todas as colunas existem.');
