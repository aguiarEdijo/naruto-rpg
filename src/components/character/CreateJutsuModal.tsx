...
import type { JutsuRank, JutsuCategory, JutsuCategoryRank, JutsuEffect } from '@/types/gameRules';
...
export const CreateJutsuModal: React.FC<CreateJutsuModalProps> = ({
    ...
    const { data: categoryRanks = [], isLoading: loadingRanks } = useJutsuCategoryRanksByCategory(selectedCategoryId);
    ...
    const handleSubmit = () => {
        ...
        const selectedEffectsData = selectedEffectIds.map(id => {
            const effect = effects.find(e => e.id === id);
            return effect ? { id: effect.id, name: effect.name, custo: effect.custo } : null;
        }).filter(Boolean) as Array<{ id: string; name: string; custo: number }>;
        ...
        onSubmit({
            ...
            rank: selectedRank,
            description,
            selectedEffects: selectedEffectsData,
            peUsed,
            peAvailable,
            chakraCost: selectedRankData?.custo || null,
            requisito: selectedRankData?.requisito || null,
            mult: selectedRankData?.mult || null,
            duracao: selectedRankData?.duracao || null,
            realidade: selectedRankData?.realidade || null,
        });
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                            ...
                            {loadingCategories ? (
                                ...
                                <div className="px-3 py-2 border border-red-300 rounded-md bg-red-50 text-red-700 text-sm">
                                    ...
                                    <ul className="text-xs list-disc list-inside mt-1">
                                        <li>As tabelas jutsu_categories foram criadas</li>
                                        <li>Os dados foram populados com o script seed-jutsu-categories.sql</li>
                                        <li>Há conexão com o banco de dados</li>
                                    </ul>
                                </div>
                            ) : (
                                <>
                                    <select
                                        ...
                                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary"
                                    >
                                        <option value="">Selecione uma categoria ({categories.length} disponíveis)</option>
                        ...
                        <div>
                            ...
                            {loadingRanks || !selectedCategoryId ? (
                                <div className="px-3 py-2 border border-gray-300 rounded-md bg-gray-100">
                                    {!selectedCategoryId ? 'Selecione uma categoria primeiro' : 'Carregando...'}
                                </div>
                            ) : (
                                <select
                                    value={selectedRank}
